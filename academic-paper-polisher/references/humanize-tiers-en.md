...
4. Keep technical terms as-is (unless heavy tier allows D5 substitution)
5. **Preserve inline citation markers** (e.g., `[6]`, `[7][8]`, `[12]-[14]`) exactly as they appear — do not change, reorder, drop, or add citation numbers
6. **Do not reduce total word count** — output should be roughly the same length as input or slightly longer. When restructuring sentences, expand elsewhere to compensate
7. **Produce humanize_matrix.md in parallel**: one row per writing unit processed
...
Every row must fill all 8 columns. Severity: High (>50%) / Medium (20-50%) / Low (<20%). Detection Dim: D1-D5.
...
For English LaTeX de-AI, do not force changes if the text is already natural
and well-written. In that case, keep the original and report:

```
[检测通过] Original text reads naturally with no obvious AI markers. No changes needed.
```
